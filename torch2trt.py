...
def torch2trt(module,
    ...
    if use_onnx:
        ...
        import onnx
        print('start onnx conversion: Note this should apear twice first time for encoder, second for decoder')
        ...
        torch.onnx.export(
            module_flat, 
            inputs_flat, 
            onnx_path,  # Save directly to a file path
        ...
        gs.export_onnx(onnx_graph, onnx_path)  # Save the modified graph back to the file path
        ...
        print('end of onnx conversion : Note this should apear twice first time for encoder, second for decoder')
        # Optionally, clean up the file if not needed
        os.remove(onnx_path)
